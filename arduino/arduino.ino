...
byte is_lock = 0;
byte is_printstate_door_lcd = 0;
String ID = "";

const String fullname = "Dang Thanh Nhan";
const String mssv = "3119510032";
const byte LedPins [4] = {4, 5, 6, 7};  // Chân led 1
...
void Start() {
  read_sensor();
  updateState(1, 1, 1); //id, iddv, idtp
  print_value_lcd();
...
void updateState(byte id, byte idc, byte idtp) {
  ...
  jsonBuffer["r1"] = digitalRead(LedPins[0]);
  jsonBuffer["r2"] = digitalRead(LedPins[1]);
  jsonBuffer["r3"] = digitalRead(LedPins[2]);
  jsonBuffer["r4"] = digitalRead(LedPins[3]);
